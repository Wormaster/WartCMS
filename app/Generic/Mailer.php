...
class Mailer {
    ...
    function callmail($message, $title = null){
        if (!$title){
            $title = mb_convert_encoding('Сообщеие с сайта - '.$this->reg['sitename'], "CP1251", "UTF-8");
        }
        $to = $this->reg['adminmail'];
        $sendmessage = mb_convert_encoding('Уважаемый администратор,
                    '.$message.'
        ', "CP1251", "UTF-8");
        try {
            // minimal requirements to be set
            self::setFrom($this->reg['sitename'], $this->reg['adminmail']);
            self::addRecipient(null,$to);
            self::fillSubject($title);
            self::fillMessage($sendmessage);
