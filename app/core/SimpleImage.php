...
class SimpleImage {
	...
	//
    public function fill_to_width($width, $height, $color) {
        $new_image = imagecreatetruecolor($width, $height);
        $rgb = $this->hex2rgb($color);
        $fill = imagecolorallocate($new_image, $this->keep_within($rgb['r'], 0, 255), $this->keep_within($rgb['g'], 0, 255), $this->keep_within($rgb['b'], 0, 255));
        imagefill($new_image, 0, 0, $fill);

        $origw = $this->get_width();
        $origh = $this->get_height();

        $x0 = ($width - $origw)/2;

        imagecopy($new_image, $this->image, $x0, 0, 0, 0, $origw, $origh);
